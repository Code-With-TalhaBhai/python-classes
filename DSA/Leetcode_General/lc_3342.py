

def minTimeToReach1(moveTime) -> int:
    import heapq
    m = len(moveTime)
    n = len(moveTime[0])
    min_heap = [(0,0,0)]

    visited = set()
    visited.add((0,0))

    while min_heap:
        t,x,y = heapq.heappop(min_heap)
        if x == m - 1 and y == n - 1:
            return t


        top = x - 1
        left = y - 1
        bottom = x + 1
        right = y + 1

        if top > -1 and (top,y) not in visited:
            mt = (top + y) % 2
            if mt == 0:
                mt = 2
            maxi = max(t,moveTime[top][y]) + mt
            heapq.heappush(min_heap,(maxi,top,y))
            visited.add((top,y))
        if left > -1 and (x,left) not in visited:
            mt = (x + left) % 2
            if mt == 0:
                mt = 2
            maxi = max(t,moveTime[x][left]) + mt
            heapq.heappush(min_heap,(maxi,x,left))
            visited.add((x,left))
        if bottom < m and (bottom,y) not in visited:
            mt = (bottom + y) % 2
            if mt == 0:
                mt = 2
            maxi = max(t,moveTime[bottom][y]) + mt
            heapq.heappush(min_heap,(maxi,bottom,y))
            visited.add((bottom,y))
        if right < n and (x,right) not in visited:
            mt = (x + right) % 2
            if mt == 0:
                mt = 2
            maxi = max(t,moveTime[x][right]) + mt
            heapq.heappush(min_heap,(maxi,x,right))
            visited.add((x,right))
    return -1



def minTimeToReach2(moveTime) -> int:
    import heapq
    m = len(moveTime)
    n = len(moveTime[0])
    min_heap = [(0,0,0)]

    min_time = [[float('inf')]*n for _ in range(m)]
    min_time[0][0] = 0
    directions = [(-1,0),(0,-1),(0,1),(1,0)]

    while min_heap:
        t,x,y = heapq.heappop(min_heap)
        if x == m - 1 and y == n - 1:
            return t
        
        if t > min_time[x][y]:
            continue

        for dx,dy in directions:
            nx = dx + x
            ny = dy + y

            if nx >= 0 and nx < m and ny >= 0 and ny < n:
                mt = (nx + ny) % 2
                if mt == 0:
                    mt = 2
                maxi = max(t,moveTime[nx][ny]) + mt
                if maxi < min_time[nx][ny]:
                    min_time[nx][ny] = maxi
                    heapq.heappush(min_heap,(maxi,nx,ny))
    return -1


moveTime1 = [[0,4],[4,4]]
moveTime2 = [[0,0,0,0],[0,0,0,0]]
moveTime3 = [[0,1],[1,2]]


print(minTimeToReach1(moveTime1))
print(minTimeToReach1(moveTime2))
print(minTimeToReach1(moveTime3))
print()
print(minTimeToReach2(moveTime1))
print(minTimeToReach2(moveTime2))
print(minTimeToReach2(moveTime3))